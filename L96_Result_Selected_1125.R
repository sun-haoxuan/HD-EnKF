...
result = foreach(i = 1:nrow(param), .combine = 'rbind') %do%
  {
    ...
    load(file = paste0('Output/L96_test/L96_set25_taper-ev-p_scale(2)/L96_p', p, '_q', q, '_n', n, '_seq', S.seq, '/',
                       mt, '_F', f, '_B', b, '.Rdata'))
    data.frame(
      p = p, n = n,
      method = mt,
...
g = result %>%
  ...
  theme(legend.title = element_blank(),
        legend.position = 'bottom')
print(g)
ggsave('Output/L96_final_1125/Result/result-all_RMSE_6.png', g, width = 8, height = 8, dpi = 600)
...
g = foreach(mt = c('standard', 'oracle', 'inflation', 'GC-only', 'GC-ev-p'), .combine = 'rbind') %do%
  {
    if(mt == 'GC-only'){
      ...
      b = rmse.all %>% arrange(RMSE) %>% slice(1) %>% pull(boot)
      load(paste0('Output/L96_test/L96_set26_taper-only/L96_p', p, '_q', q, '_n', n, '_seq4/', mt, '_F', f, '_B', b, '.Rdata'))
    }else if(mt == 'GC-ev-p'){
      rmse.all = foreach(b = 1:10, .combine = 'rbind') %do%
        {
          load(file = paste0('Output/L96_test/L96_set25_taper-ev-p_scale(2)/L96_p', p, '_q', q, '_n', n, '_seq', S.seq, '/',
      ...
      b = rmse.all %>% arrange(RMSE) %>% slice(1) %>% pull(boot)
      load(paste0('Output/L96_test/L96_set25_taper-ev-p_scale(2)/L96_p', p, '_q', q, '_n', n, '_seq4/', mt, '_F', f, '_B', b, '.Rdata'))
    }else{
      load(paste0('Output/L96_final_1125/L96_p', p, '_q', q, '_n', n, '_seq4/', mt, '_F', f, '.Rdata'))
    }
...
g = dat.plot %>% 
  ...
  theme(legend.title = element_blank(),
        legend.position = 'bottom')
print(g)
ggsave('Output/L96_final_1125/Result/result_RMSE_t_5.png', g, width = 5, height = 7.5, dpi = 600) 
...
dat.plot = foreach(p = c(40, 100, 200), .combine = 'rbind') %do%
  {
    ...
    tmp1 = foreach(f = 4:12, .combine = 'rbind') %do%
      {
        mt = 'GC-constant'
        dir.open = paste0('Output/L96_test/L96_set27_taper-constant/L96_p', p, '_q', q, '_n', n, '_seq', S.seq, '/')
        # rmse.all = foreach(b = 1:10, .combine = 'rbind') %do%
        #   {
        #     load(paste0(dir.open, mt, '_F', f, '_B', b, '.Rdata'))
        #     data.frame(
    ...
    tmp2 = foreach(f = 4:12, .combine = 'rbind') %do%
      {
        mt = 'GC-ev-p'
        dir.open = paste0('Output/L96_test/L96_set25_taper-ev-p_scale(2)/L96_p', p, '_q', q, '_n', n, '_seq', S.seq, '/')
        rmse.all = foreach(b = 1:10, .combine = 'rbind') %do%
          {
            load(paste0(dir.open, mt, '_F', f, '_B', b, '.Rdata'))
            data.frame(
...
g = dat.plot %>%
  ...
  theme(legend.position = 'bottom',
        legend.title = element_blank())
print(g)
ggsave('Output/L96_final_1125/Result/result-smoothBW_4.png', g, width = 7, height = 2.5, dpi = 600)
