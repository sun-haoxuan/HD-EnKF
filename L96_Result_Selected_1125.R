...
for(p in c(40, 100, 200)){
  for(n in c(20, 30, 40)){
    q = p; S.seq = 4
    dir.save = paste0('L96_taper-only/L96_p', p, '_q', q, '_n', n, '_seq', S.seq, '/')
    dir.create(dir.save)
...
## adjust on F = 8
read.csv("L96_final_1125/Result/result-all_1129.csv") %>% filter(p == 40 & n == 30 & F.set == 8)
p = q = 40; n = 30; S.seq = 4
dir.open = paste0('L96_test/L96_set16_taperonly_param2/L96_p', p, '_q', q, '_n', n, '_seq', S.seq, '/')
dir.save = paste0('L96_taper-only/L96_p', p, '_q', q, '_n', n, '_seq', S.seq, '/')
resultall('L96_test/L96_set16_taperonly_param2/',  40, 40, 30, 4, 'GC', 8, 10)
...
## plot
p = q = 40; n = 30; S.seq = 4
dir1 = paste0('L96_final_1125/L96_p', p, '_q', q, '_n', n, '_seq', S.seq, '/')
dir2 = paste0('L96_taper-only/L96_p', p, '_q', q, '_n', n, '_seq', S.seq, '/')
result = data.frame()
for(mt in c('banding', 'tapering', 'GC')){
...
g = result %>%
  ...
  theme(legend.title = element_blank(),
        legend.position = 'bottom')
print(g)
ggsave('L96_taper-only/result-taper-only.png', g, width = 9, height = 3.5, dpi = 600)
...
for(p in c(40, 100, 200)){
  ...
  n = 30
  S.seq = 4
  dir.open1 = paste0('L96_final_1125/L96_p', p, '_q', q, '_n', n, '_seq', S.seq, '/')
  dir.open2 = paste0('L96_smoothBW/L96_p', p, '_q', q, '_n', n, '_seq', S.seq, '/')
  for(mt in c('banding', 'tapering', 'GC')){
...
print(g)
ggsave('L96_smoothBW/result-smoothBW.png', g, width = 8, height = 3.5, dpi = 600)
